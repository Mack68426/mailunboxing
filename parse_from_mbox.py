...
mailbox_outdir = "emails" # store .eml files dsorted by mbox
...
def extract_tarfile(file_path, out_dir=".", exist_ok = False):
...
def mbox_to_eml(mboxfile_path, out_dir='.', exist_ok = False) -> None:
...
# test function for extracting tar.gz data file
def test_extract_tar_file():
    tar_path = f"{resource_dir}/{tar_file}" # 壓縮檔路徑

    # 將壓縮資料檔案解壓縮到目地資料夾
    # extract the mbox files in tar.gz to target directory
    extract_tarfile(tar_path, resource_dir, exist_ok=True)

# almost main function (previously)
def parse_from_mbox():
    ...
    for mbox_file in mboxfiles:
        ...
        mbox_to_eml(mbox_file, f"{mailbox_outdir}/{list_name}", exist_ok=True)
